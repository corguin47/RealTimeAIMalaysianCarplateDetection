...
class lowlight_enhance(object):
    def __init__(self, sess):
        ...
        self.R_low, self.I_low = DecomNet(self.input_low, layer_num=5)  # <-- fix: pass layer_num
        self.I_delta = RelightNet(self.I_low, self.R_low)              # <-- fix: pass input_L and input_R

        self.output_S = self.R_low * self.I_delta

        self.output_R_low = self.R_low
        self.output_I_low = self.I_low
        self.output_I_delta = self.I_delta

        self.global_vars = tf.global_variables()
        self.saver = tf.train.Saver(var_list=self.global_vars)

        self.sess.run(tf.global_variables_initializer())
        print("[*] Initialize model successfully...")

    def inference(self, input_low):
        return self.sess.run(self.output_S, feed_dict={self.input_low: input_low})
